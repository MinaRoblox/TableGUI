...
class TableGUI:
    def __init__(self,
        ...
        def return_table():
            return [
                [self.background_color for x in range(table_size)] for y in range(table_size)
            ]
        # window code
        # screen code
        self.all_screens = []
    ...
    def render_image(self, image_path: str, image_size: int, image_coordinates: tuple, image_background: tuple, screen: int):
        """
        BY THE WAY, MAKE SURE THAT YOUR IMAGE DOES NOT HAVE THE USE THE SAME COLOR  AS YOUR BACKGROUND COLOR
        image_path: path to image
        image_size: the size of the image (image_size x image_size) in pixels
        image_coordinates: the position of where the image will be drawn
        image_background: the color of the image that will be flagged as background color
        screen: the selected screen you want to draw
        """
        ...
        for y in range(image_size):
            y += image_coordinates[1]
            for x in range(image_size):
                x += image_coordinates[0]
                coordenadas.append((x, y))
        for coordenda in coordenadas:
            if self.debug:
                print(f"Color: {color[i]}")
                print(f"Coordenada: {coordenda}")
            if color[i] == self.background_color or color[i] == image_background:
                if (i + 1) == (image_size * image_size):
                    break
                i += 1
                continue
